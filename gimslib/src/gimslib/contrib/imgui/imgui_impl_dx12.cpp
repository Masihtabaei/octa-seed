...
void ImGui_ImplDX12_RenderDrawData(ImDrawData* draw_data, ID3D12GraphicsCommandList6* ctx)
{
  ...
  if (fr->VertexBuffer == NULL || fr->VertexBufferSize < draw_data->TotalVtxCount)
  {
    ...
    if (bd->pd3dDevice->CreateCommittedResource(&props, D3D12_HEAP_FLAG_NONE, &desc, D3D12_RESOURCE_STATE_GENERIC_READ,
                                                NULL, IID_PPV_ARGS(&fr->VertexBuffer)) < 0)
      return;
  }
  if (fr->IndexBuffer == NULL || fr->IndexBufferSize < draw_data->TotalIdxCount)
  {
    ...
    desc.Flags            = D3D12_RESOURCE_FLAG_NONE;
    if (bd->pd3dDevice->CreateCommittedResource(&props, D3D12_HEAP_FLAG_NONE, &desc, D3D12_RESOURCE_STATE_GENERIC_READ,
                                                NULL, IID_PPV_ARGS(&fr->IndexBuffer)) < 0)
      return;
  }
...
static void ImGui_ImplDX12_CreateFontsTexture()
{
  ...
  // Upload texture to graphics system
  {
    ...
    bd->pd3dDevice->CreateCommittedResource(&props, D3D12_HEAP_FLAG_NONE, &desc, D3D12_RESOURCE_STATE_COPY_DEST, NULL,
  ...
  // file!) [Solution 4] command-line: add '/D ImTextureID=ImU64' to your cl.exe command-line (this is what we do in the
  // example_win32_direct12/build_win32.bat file)
...
bool ImGui_ImplDX12_CreateDeviceObjects()
{
  ...
  // Create the vertex shader
  {
    ...
    if (FAILED(D3DCompile(vertexShader, strlen(vertexShader), NULL, NULL, NULL, "main", "vs_5_0", 0, 0,
  ...
  // Create the pixel shader
  {
    ...
    if (FAILED(D3DCompile(pixelShader, strlen(pixelShader), NULL, NULL, NULL, "main", "ps_5_0", 0, 0, &pixelShaderBlob,
                          NULL)))
    {
